...
export const { reducer: postsReducer, actions } = createSlice({
  ...
  reducers: {
    ...
    removePostFromList: (state, { payload }: PayloadAction<PostsActionPayloads['deletePost']>) => {
      state.list.allIds = state.list.allIds.filter((postId) => postId !== payload.id)
      delete state.list.byId[payload.id]
    },
  },
...
export const { initPosts, addPost, editPost, deletePost, removePostFromList } = actions
