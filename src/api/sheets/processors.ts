    ...
    const processed = response.values.reduce((acc, row) => {
        ...
        switch (fieldName as keyof ResumeSlice) {
            ...
            case 'contributions': {
                ...
                items.push({
                    assetUrl, description, imageUrl, title
                })
                break;
            }
            case 'education': {
                ...
                items.push({
                    company, from, location, position, to
                })
                break;
            }
            case 'experience': {
            ...
            case 'publications': {
                const [, title, journal, date, url] = row;
                const items = acc[fieldName] as ResumeSlice['publications']
                items.push({
                    date, journal, title, url
                })
                break;
            }
            case 'skills': {
            ...
            case 'sectionNames': {
                ...
                const items = acc[fieldName] as ResumeSlice['sectionNames']
